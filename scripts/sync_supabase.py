"""
Upsert scored leads into the Supabase leads table.

Loads scored_leads.csv and upserts all rows using acct_number as the
conflict key. Requires SUPABASE_URL and SUPABASE_ANON_KEY in .env.
"""
...
INPUT_PATH = os.path.join(SCRIPTS_DIR, "scored_leads.csv")
...
def main():
    ...
    result = (
        client.table("leads")
        .select("acct_number, property_address, owner_name, lead_score")
    ...
    for i, lead in enumerate(result.data, 1):
        print(
            ...
            f"{lead['property_address']} ({lead['owner_name']})"
        )
